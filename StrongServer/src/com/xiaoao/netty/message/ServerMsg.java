 ...
 * @Author: siyunlong
 * @Version: V1.00
 * @Create Date: 2015-5-8下午3:24:40
 */
public class ServerMsg {

    private static final int INIT_MSG_LEN = 1024;
    private static final int COMM_COMPRESS_SIZE = 3000;
...
//        logString.append("[Byte:").append(b).append("]");
    ...
    public void appendUnit(ServerMsg msg) {
...
//            logString.append("[String:").append(s).append("]");
    ...
    public void appendBytes(byte[] arrByte) {
        try {
            int nLen = arrByte.length;
            ensureCapacity(nLen + MsgConstant.INT_SIZE);
            System.arraycopy(BitConverter.getBytes((int) nLen), 0, m_aBuff, m_nLen, MsgConstant.INT_SIZE);
            m_nLen += MsgConstant.INT_SIZE;
...
//            m_nLen = m_nPos;
            logString.append("[").append(Arrays.toString(arrByte)).append("]");
//            logString.append("[String:").append(s).append("]");
        } catch (Exception ex) {
        	ex.printStackTrace();
        }
